...
namespace transport {
	namespace render {
        ...
        const std::set<const domain::Stop*, domain::StopCompare> MapRenderer::AllStops(const std::deque<const domain::Bus*>& routes, std::unordered_set<geo::Coordinates, geo::CoordinateshHasher>& allCoord) {
        ...
        void MapRenderer::Draw(std::ostream& out, std::deque<const domain::Bus*> routes) {
            ...
            const std::set<const domain::Stop*, domain::StopCompare> allStops = AllStops(routes, allCoord);
            ...
            const domain::SphereProjector proj{ allCoord.begin(), allCoord.end(), width, height, padding };
            svg::Document doc;
            size_t colorIndex = 0;
